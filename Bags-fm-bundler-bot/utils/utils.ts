...
export const saveLUTFile = (newData: string, filePath: string = "lut.json") => {
  try {
    ...
    // Add the new data as a single string to the array
    existingData.unshift(newData);

    // Write the updated data back to the file
    fs.writeFileSync(filePath, JSON.stringify(existingData, null, 2));
    console.log("LUT address saved successfully.");
    ...
    try {
      ...
      // Create new file with the address as a single string in an array
      fs.writeFileSync(filePath, JSON.stringify([newData], null, 2));
      console.log("LUT file is saved successfully.")
    } catch (error) {
      console.log('Error saving LUT address to JSON file:', error);
    }
  }
};

export const saveNewFile = (newData: Data[], filePath: string = "data.json") => {
  try {
    // Write the updated data back to the file
    fs.writeFileSync(filePath, JSON.stringify(newData, null, 2));
...
export function readLUT(filename: string = "lut.json"): Data[] {
  if (!fs.existsSync(filename)) {
    // If the file does not exist, create an empty array
    fs.writeFileSync(filename, '[]', 'utf-8');
  }
